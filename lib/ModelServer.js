...
function ToTypedArray(arraybuffer, type) {
    try {
...
module.exports = class ModelServer {
    constructor(modelName, modelPath, onDoneTraining) {
        ...
        this.name = modelName;
        this.w_path = modelPath.substring(1);
        this.path = modelPath;
    ...
    mergeAll(clientsToMerge, save) {
        ...
        let maxStep = step;
        let toMerge = ToTypedArray(this.toMerge.slice(8), type);
        this.toMerge = null;
        ...
        for (let id of list) {
            ...
            if (client.weights && client.weights !== 'validating') {
                ...
                step += s;
                let weights = ToTypedArray(client.weights.slice(8), type);
                // average the weights
    ...
    getData() {
        return this.training.nextTrainBatch(this.config.training.batch_size);
    }

    getValidationData() {
        return this.validation.nextTestBatch(this.config.validation.batch_size);
    }
    ...
    onValidation(client_id, arraybuffer, networkTime) {
        let metrics = new Float32Array(arraybuffer, 0, 2); // metrics = [ loss, accuracy ]
