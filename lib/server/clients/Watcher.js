...
function updateCharts(data) {
	...
	if (data.validation.loss.length || data.validation.accuracy.length) {
		if (data.validation.loss.length)
			Array.prototype.push.apply(validationChart.data.datasets[0].data, data.validation.loss);

		if (data.validation.accuracy.length)
			Array.prototype.push.apply(validationChart.data.datasets[1].data, data.validation.accuracy);
...
let updateLoop = window.setInterval(getUpdate, validation_rate * 1000);
