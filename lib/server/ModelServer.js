...
const updateRunningAverage = function (loss, average) {
    // update running averages
    validationAverage.data.push({ loss, accuracy });
    if (validationAverage.data.length > 10)
    ...
    validationAverage.loss /= 10;
    validationAverage.accuracy /= 10;
}
...
module.exports = class ModelServer {
    ...
    putWeights(client_id, weights) {
        ...
        if (this.validateNext) {
            ...
            if (this.config.validation.merge_all)
                return Buffer.concat([
                    Buffer.from(new Float32Array([0]).buffer),
        ...
        // check for special case where only one client is training
    ...
    mergeAll(list, save) {
        ...
        let maxStep = step;
        let toMerge = INFLATE(this.toMerge.slice(4).buffer, type);
        this.toMerge = null;
        ...
        for (let client of list) {
            if (client.weights && client.weights !== 'validating') {
                ...
                step += s;
                let weights = INFLATE(client.weights.slice(4).buffer, type);
                // do weighted average on the weights
        ...
        this.toMerge = Buffer.concat([
            Buffer.from(new Float32Array([maxStep]).buffer),
            DEFLATE(toMerge)
        ]);
        if (save)
    ...
    getWatcherData() {
        ...
        this.watcherData = { validation: [], loss: [], accuracy: [] };

        if (this.shouldEndTraining)
            window.setTimeout(this.onDoneTraining, 100);
    ...
    onMetrics(client, arraybuffer, networkTime) { 
        ...
        if (this.merging[client].weights === 'validating') {
            this.merging[client].weights = null;
            ...
            FS.appendFile(this.w_path + 'training.csv', `${networkTime.now},${client},${loss},${accuracy}\n`, 'utf8', (error) => (
