...
function modelSize(model) {
    var result;
    var prev = null;
    for (var layer of model) {
        if (prev && layer.output === undefined) {
            result += layer.out * (prev.input ||prev.out) + (layer.bias ? layer.out : 0);
        } else prev = layer;
    }
}
...
module.exports = class ModelServer {
    constructor(modelPath, onDoneTraining) {
        ...
        this.validator = new Validator([].concat(this.model), this.config.validation.data_delegate, this.config.byte_weights, this.currentWeights);
    ...
    validate() {
        if (this.config.validation.merge_all) {
            ...
            // let type = this.config.byte_weights ? 'Int8Array' : 'Float32Array';
            this.validator.validateWeights(this.toMerge, this.afterValidation );
        }
    }
